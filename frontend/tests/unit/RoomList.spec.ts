// RoomList.spec.ts の一時的なテスト
import { shallowMount } from '@vue/test-utils';
import RoomList from '@/components/RoomList.vue'; // @ を使うように修正

describe('Simple RoomList Mount Test', () => {
  it('mounts the component', () => {
    // apiService.getRooms をモック化 (これがないと onMounted でエラーになる可能性)
    jest.mock('@/services/apiService', () => ({
      getRooms: jest.fn().mockResolvedValue([]), // 空の配列を返すように
    }));
